"""
GRAVEC v2 + Agnes AI — MATH-500 36题基准测试

使用 Agnes 2.0 Flash 模型，对从 MATH-500 全集中分层选取的 36 道题
（覆盖 L1-L5，共 7 个学科），运行 GRAVEC v2 (reason_from_future_nhx) 进行测试。

题目选择规则（见 scripts/select_numeric_math500.py）：
    L1: 5, L2: 7, L3: 8, L4: 8, L5: 8 = 36 道
    仅选最终答案可解析为 float 的题（数字、分数）

运行方式：
    cd /Users/mac/WorkBuddy/Claw/rff-enhanced
    PYTHONPATH=src uv run python tests/benchmark_math500.py
"""
...
def main():
    # 加载 36 题数据集
    data_path = os.path.join(os.path.dirname(__file__), "..", "math500_36_numeric.jsonl")
    with open(data_path) as f:
        selected = [json.loads(line) for line in f]

    print(f"\n{'='*70}")
    print(f"GRAVEC v2 + Agnes 2.0 Flash — MATH-500 36题基准测试")
    print(f"模型: openai/agnes-2.0-flash")
    print(f"题目: {len(selected)} 道 (L1:5 + L2:7 + L3:8 + L4:8 + L5:8)")
    ...
    for i, problem_data in enumerate(selected):
        print(
            f"[{i+1:2d}/36] L{problem_data['level']} {problem_data['subject']:20s} "
    ...
    for subj, subset in sorted(by_subject.items()):
        c = sum(1 for r in subset if r["correct"])
        print(f"    {subj:25s}: {c}/{len(subset)} ({c/len(subset)*100:.0f}%)")
    ...
    if errors:
        ...
        for r in errors:
            ...
            print(f"  L{r['level']} seq={r['seq']:2d} {r['subject']:20s} "
                  f"got={answer_str:>12s} gold={r['gold']:>12g}{err_str}")

    # 保存结果
    output_path = os.path.join(os.path.dirname(__file__), "..", "math500_gravec_results.json")
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump({
            "model": "openai/agnes-2.0-flash",
            "engine": "GRAVEC v2 (reason_from_future_nhx)",
            "dataset": "MATH-500 (numeric subset)",
            "total": len(results),
